...
function printHelp() {
  ...
  console.log(`  target-dir   Direktori tujuan instalasi (default: direktori saat ini)\n`);
  console.log(`${c.bold}OPTIONS${c.reset}`);
  console.log(`  --force      Timpa file yang sudah ada tanpa konfirmasi`);
  console.log(`  --global     Install ke ~/.claude/ (berlaku di semua project)`);
  console.log(`  --help       Tampilkan pesan ini\n`);
  console.log(`${c.bold}CONTOH${c.reset}`);
  console.log(`  npx claude-setup                    # Install ke direktori saat ini`);
  console.log(`  npx claude-setup ./my-project       # Install ke direktori tertentu`);
  console.log(`  npx claude-setup --force            # Timpa tanpa konfirmasi`);
  console.log(`  npx claude-setup --global           # Install ke ~/.claude/\n`);
}
...
async function installToProject(targetDir, sourceDir, force) {
  ...
  if (conflicts.length > 0 && !force) {
    console.log(`${c.yellow}  ⚠ File berikut sudah ada di ${c.bold}${targetDir}${c.reset}${c.yellow}:${c.reset}`);
    for (const { label } of conflicts) dim(label);
    console.log();
    const answer = await prompt(`  Timpa semua? ${c.dim}(y/N)${c.reset} `);
...
async function installToGlobal(sourceDir, force) {
  ...
  if (hasConflicts && !force) {
    console.log(`${c.yellow}  ⚠ File berikut sudah ada di ${c.bold}~/.claude/${c.reset}${c.yellow}:${c.reset}`);
...
async function main() {
  ...
  if (isGlobal) {
    ...
    console.log();
    console.log(`${c.bold}${c.green}  Instalasi global selesai!${c.reset}`);
    console.log();
    dim(`${ruleCount} rules  •  ${cmdCount} command files`);
    console.log();
    inf(`Rules aktif otomatis di semua project`);
    inf(`Slash commands tersedia:`);
    console.log();
    for (const cmd of listCommands(path.join(globalClaudeDir, 'commands'))) {
      dim(cmd);
    }
    console.log();
    inf(`Buka Claude Code di project manapun dan rules langsung aktif`);
    console.log();
    return;
  }
  ...
  if (!fs.existsSync(targetDir)) {
    err(`Direktori tidak ditemukan: ${targetDir}`);
    console.log();
    console.log(`  Buat direktori dulu: ${c.dim}mkdir -p ${targetArg}${c.reset}`);
    console.log();
    process.exit(1);
  }
  ...
  console.log();
  console.log(`${c.bold}${c.green}  Instalasi selesai!${c.reset}`);
  console.log();
  dim(`${ruleCount} rules  •  ${cmdCount} command files`);
  console.log();
  inf(`Slash commands tersedia:`);
  ...
  console.log();
  inf(`Buka Claude Code di ${c.bold}${targetDir}${c.reset}`);
